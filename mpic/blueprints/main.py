...
from mpic.models import Photo, Tag, Comment, Collect, Notification, Follow, User
from mpic.utils import rename_image, resize_image, flash_errors, redirect_back
from mpic.forms.main import DescriptionForm, TagForm, CommentForm
from mpic.notifications import push_comment_notification, push_collect_notification
...
def delete_tag(photo_id, tag_id):
    ...
    if current_user != photo.author and not current_user.can('MODERATE'):
        abort(403)
    photo.tags.remove(tag)  # 只和对应photo解除关系而不删除tag本身
    db.session.commit()

    if not tag.photos:
        db.session.delete(tag)
        db.session.commit()  # 如果检测到该tag未和任何photo有关联，则删除
...
def show_tag(tag_id, order):
    ...
    photos = pagination.items  # 按默认按时间排序

    if order == 'by_collects':
        photos.sort(key=lambda x: len(x.collectors), reverse=True)  # 按收藏数排序
        order_rule = 'collects'
...
def search():
    ...
    if q == '':
        flash('请输入关于照片，用户或标签的关键词', 'warning')
        return redirect_back()

    category = request.args.get('category', 'photo')
    page = request.args.get('page', 1, type=int)
    per_page = current_user.config['SEARCH_RESULT_PER_PAGE']
    if category == 'user':
